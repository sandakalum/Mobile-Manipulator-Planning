...
PointCloud::Ptr leaf_cell_model(new pcl::PointCloud<pcl::PointXYZ>);
PointCloud::Ptr cast1_mold_model(new pcl::PointCloud<pcl::PointXYZ>);
PointCloud::Ptr box_model(new pcl::PointCloud<pcl::PointXYZ>);
PointCloud::Ptr leaf_cell_centroid(new pcl::PointCloud<pcl::PointXYZ>);
PointCloud::Ptr leaf_cell_centroid2(new pcl::PointCloud<pcl::PointXYZ>);

PointCloud::Ptr bowl(new pcl::PointCloud<pcl::PointXYZ>);
PointCloud::Ptr fuel_can(new pcl::PointCloud<pcl::PointXYZ>);
PointCloud::Ptr fuel_tank_tap(new pcl::PointCloud<pcl::PointXYZ>);
PointCloud::Ptr water_tap(new pcl::PointCloud<pcl::PointXYZ>);
...
void point_cloud_callback(const PointCloud::ConstPtr&  msg)
{
    ...
    //ROS_INFO_STREAM("PointCloud Updated");
}

bool recognise_callback(kuka_msgs::PoseExtraction::Request& request, kuka_msgs::PoseExtraction::Response& recognise_res)
{
    ...
    if(cloud->size() > 0){
        ...
        //std::string directory = "/home/hector/catkin_ws/src/rsd/data/" + request.object;
        //writer.write<pcl::PointXYZRGB>(directory, *cloud_p, false);  
        ...
        if(!inverted){
            ...
            copyPointCloud(*(model_map.at(request.object)), *cloud_model);
            ROS_INFO_STREAM("model caputre");
        }else
        {
            copyPointCloud(*cloud_p,*cloud_model);
            ROS_INFO_STREAM("Scene caputre");
            copyPointCloud(*(model_map.at(request.object)), *cloud_scene);
            ROS_INFO_STREAM("model caputre");
        }
    ...
    //  Output results
        ...
        if(visualisation){
            ...
            // }
            // viewer.close();
            std::shared_ptr<pcl::visualization::PCLVisualizer> viewer(new pcl::visualization::PCLVisualizer("Correspondence Grouping"));
        ...
        // tf::Transform model2baseFootprint = camera2baseFootprint*(model2camera);
...
{
    ros::init(argc, argvv, "rsd_detection");
    ros::NodeHandle nh;
    //tf2_ros::Buffer tfBuffer;
    tf2_ros::TransformListener tfListener(tfBuffer);
    ...
    ros::Subscriber point_cloud_sub = nh.subscribe<PointCloud>("camera/depth/color/points",2,point_cloud_callback);
    ros::ServiceServer rsd_server = nh.advertiseService("/rsd_detect", recognise_callback);
    ...
    std::vector<std::string> names = {"leaf_cell_model_new.pcd","cast1_mold_model_new.pcd",
                                        "box_model_new.pcd","leaf_cell_centroid.pcd","leaf_cell_centroid2.pcd"};
    std::vector<std::string> new_names = {"bowl_sampled.pcd", "fuel_can_sampled.pcd", "fuel_tank_tap_sampled.pcd","water_tap_sampled.pcd" };

    // pcl::io::loadPCDFile<pcl::PointXYZ>(directory + names[0], *leaf_cell_model);
    // pcl::io::loadPCDFile<pcl::PointXYZ>(directory + names[1], *cast1_mold_model);
    // pcl::io::loadPCDFile<pcl::PointXYZ>(directory + names[2], *box_model);
    pcl::io::loadPCDFile<pcl::PointXYZ>(directory + names[3], *leaf_cell_centroid);
    pcl::io::loadPCDFile<pcl::PointXYZ>(directory + names[4], *leaf_cell_centroid2);
    pcl::io::loadPCDFile<pcl::PointXYZ>(directory_rsd + new_names[0], *bowl);
    pcl::io::loadPCDFile<pcl::PointXYZ>(directory_rsd + new_names[1], *fuel_can);
    pcl::io::loadPCDFile<pcl::PointXYZ>(directory_rsd + new_names[2], *fuel_tank_tap);
    pcl::io::loadPCDFile<pcl::PointXYZ>(directory_rsd + new_names[3], *water_tap);




    // model_map.insert(std::pair<std::string, PointCloud::Ptr>("leaf_cell",leaf_cell_model));
    // model_map.insert(std::pair<std::string, PointCloud::Ptr>("cast1",cast1_mold_model));
    // model_map.insert(std::pair<std::string, PointCloud::Ptr>("box",box_model));
    model_map.insert(std::pair<std::string, PointCloud::Ptr>("leaf_cell_centroid",leaf_cell_centroid));
    model_map.insert(std::pair<std::string, PointCloud::Ptr>("leaf_cell_centroid2",leaf_cell_centroid2));
