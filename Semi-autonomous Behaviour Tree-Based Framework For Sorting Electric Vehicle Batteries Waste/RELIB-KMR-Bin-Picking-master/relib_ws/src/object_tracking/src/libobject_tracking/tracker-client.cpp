...
namespace visp_tracker
{
        ...
        resourceRetriever_()
  {
    ...
    // NOTE: Commented because modelName_ var does not come form the param server anymore, instead, it is initialized when TrackerClient is instanced (created)
    nodeHandlePrivate_.param<std::string>("model_name", modelName_, _object_name);

    nodeHandlePrivate_.param<bool>("start_from_saved_pose", startFromSavedPose_, false);

    nodeHandlePrivate_.param<bool>("confirm_init", confirmInit_, true);
    ...
    while (cameraPrefix_.empty())
    {
      ...
      {
        ROS_WARN("the camera_prefix parameter does not exist.\n"
                 "This may mean that:\n"
      ...
      {
        ROS_INFO("tracker is not yet initialized, waiting...\n"
    ...
    // Camera subscriber.
    cameraSubscriber_ = imageTransport_.subscribeCamera(rectifiedImageTopic_, queueSize_,
  ...
  TrackerClient::spin()
  {
    boost::format fmtWindowTitle("[RELIB UoB] Tracker initialization ");
    ...
    while (!ok && !exiting())
    {
      try
      {
        ...
        if (confirmInit_)
        {
          ...
          do
          {
            ...
            mutex_.unlock();
            vpDisplay::displayCharString(image_, point, "tracking, click to initialize tracker",
                                         vpColor::red);
            vpDisplay::flush(image_);
      ...
      {
        mutex_.unlock();
        ROS_ERROR_STREAM("failed 1 to initialize: "
                         << e.what() << ", retrying...");
      }
      catch (const std::string &str)
      {
        mutex_.unlock();
        ROS_ERROR_STREAM("failed 2 to initialize: "
                         << str << ", retrying...");
      }
      catch (...)
      {
        mutex_.unlock();
        ROS_ERROR("failed unkown to initialize, retrying...");
      }
    }
  ...
  TrackerClient::sendcMo(const vpHomogeneousMatrix &cMo)
  {
    ...
    {
      ROS_INFO("Waiting for the initialization service to become available.");
      rate.sleep();
    }
  ...
  TrackerClient::init()
  {
    ...
    tracker_.initFromPose(image_, cMo);
    // NOTE: Uncomment this line if you would like the postion guess as soon as the camera window prompts
    saveInitialPose(cMo);
