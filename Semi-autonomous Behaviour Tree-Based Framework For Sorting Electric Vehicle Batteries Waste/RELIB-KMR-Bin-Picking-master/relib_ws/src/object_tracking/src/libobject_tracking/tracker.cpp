...
namespace visp_tracker
{
                        ...
                        visp_tracker::Init::Response &res)
  {
    ...
    // Necesary to enter to the spin loop
    iterations = 0;
    ...
    if (trackerType_ == "mbt+klt")
    { // Hybrid Tracker reconfigure
    ...
    else if (trackerType_ == "mbt")
    { // Edge Tracker reconfigure
    ...
    tracker_.setCovarianceComputation(false);
        ...
        objectPositionHint_()
  {
    ...
    nodeHandle_.param<std::string>("frame_id", childFrameId_, "target_pose");

    // Robot motion compensation.
    nodeHandle_.param<std::string>("world_frame_id", worldFrameId_, "/odom");
    nodeHandle_.param<bool>("compensate_robot_motion", compensateRobotMotion_, false);
    ...
    // Result publisher.
    resultPublisher_ =
        nodeHandle_.advertise<geometry_msgs::PoseWithCovarianceStamped>(visp_tracker::object_position_covariance_topic, queueSize_);

    transformationPublisher_ =
        nodeHandle_.advertise<geometry_msgs::TransformStamped>(visp_tracker::object_position_topic, queueSize_);

    // Moving edge sites_ publisher.
    movingEdgeSitesPublisher_ =
        nodeHandle_.advertise<visp_tracker::MovingEdgeSites>(visp_tracker::moving_edge_sites_topic, queueSize_);

    // Klt_points_ publisher.
    kltPointsPublisher_ =
        nodeHandle_.advertise<visp_tracker::KltPoints>(visp_tracker::klt_points_topic, queueSize_);
    ...
    objectPositionHintSubscriber_ =
        nodeHandle_.subscribe<geometry_msgs::TransformStamped>("object_position_hint", queueSize_, callback);
    ...
    if (cameraParameters_.get_px() == 0. || cameraParameters_.get_px() == 1. || cameraParameters_.get_py() == 0. || cameraParameters_.get_py() == 1. || cameraParameters_.get_u0() == 0. || cameraParameters_.get_u0() == 1. || cameraParameters_.get_v0() == 0. || cameraParameters_.get_v0() == 1.)
    ...
    initService_ = nodeHandle_.advertiseService(visp_tracker::init_service, initCallback);
  }
  ...
  void Tracker::spin()
  {
    ...
    while (!exiting())
    {
       //Detection will be running unless the nummber of iterations overpass N, this is to stop and avoid detection when is not needed, also, it can cause proble
      if(iterations <= 200){
        ...
        if (lastTrackedImage_ < header_.seq)
        {
          ...
          if (state_ == LOST)
          {
            ...
            if (ros::Time::now() - objectPositionHint_.header.stamp < ros::Duration(1.))
          ...
          if (state_ == TRACKING)
          {
            ...
            {
              geometry_msgs::TransformStampedPtr objectPosition(new geometry_msgs::TransformStamped);
            ...
            {
              geometry_msgs::PoseWithCovarianceStampedPtr result(new geometry_msgs::PoseWithCovarianceStamped);
            ...
            // Publish to tf.
            transform.setOrigin(tf::Vector3(transformMsg.translation.x,
                                            transformMsg.translation.y,
                                            transformMsg.translation.z));
            transform.setRotation(tf::Quaternion(transformMsg.rotation.x,
                                                 transformMsg.rotation.y,
                                                 transformMsg.rotation.z,
                                                 transformMsg.rotation.w));
        ...
        lastHeader = header_;
      }else if (iterations == 201){
        ROS_INFO("Detection stopped! Waiting for new initialization..");
      }
      else
      {
        iterations = 204; // Just to not over flow this integer
      }
      spinOnce();
  ...
  void
  Tracker::objectPositionHintCallback(const geometry_msgs::TransformStampedConstPtr &transform)
  {
