...
// #define PLATE_NAME  "plate.pcd"
// #define COVER_NAME  "cover.pcd"
// #define SCENE_NAME  "scene_cover_plate.pcd"


// argv[0] = FEVOR_pose_estimation.exe
// argv[1] = scene
// argv[2] = object (to detect in scene)
// argv[3] = rotation to object (optional ...set default to 3)
// argv[4] = filtering (NOTE:always use filter with 0.000895 value for PLATE object) and (for cover set it to 0.0001)
// argv[5] = Matching threshold (change it when require)
// argv[6] = points for setting bounding box(CUBE)
...
namespace fevor{
    ...
    public:
        ...
        //const char* default_dir = "~/catkin_ws/src/erl_rnt/data/";
        std::string scene_name; std::string model_name; float rotation_to_object = 0; float filtering = 0; float matching_treshold = 0; int number_of_points;
        // TODO: Fix agv for the correct argument or change where arg[] is called 
    ...
    public:
        ...
        Detector(std::string _scene, std::string _obj_name, const char* rot, const char* filt, const char* thresh, const char* num);
        //Detector(std::string _scene, std::string  _obj_name, std::string  rot, std::string  filt, std::string  thresh, std::string  num);

        //~Detector();

        void pose_estimation(geometry_msgs::Pose& object, bool visualiser);


    };

    //Detector::Detector(std::string _scene, std::string  _obj_name, std::string  rot, std::string  filt, std::string  thresh, std::string  num) {
    Detector::Detector(std::string _scene, std::string _obj_name, const char* rot, const char* filt, const char* thresh, const char* num){

        scene_name = default_dir + _scene;
        model_name = default_dir + _obj_name;
        this->files.push_back("Dummy");
        this->files.push_back(scene_name);
        this->files.push_back(model_name); 
        //const char* argv_temp[7] = {"dummy", temp1, temp2, rot, filt, thresh, num};
        //ROS_INFO_STREAM("1:    " << this->files[1] << "  2:   "<<this->files[2]);
        this->argv[0] =  "dumy";
        this->argv[1] =  "dummy";//scene.c_str();
        this->argv[2] =  "dummy";//model.c_str();
        this->argv[3] =  rot;
        this->argv[4] =  filt;
        this->argv[5] =  thresh;
        this->argv[6] =  num;
    ...
    void Detector::pose_estimation( geometry_msgs::Pose& object, bool visualiser){
        time_t tstart, tend;
        tstart = time(0);
        ...
        //float centroid[1200][3], centroid_original[1200][3];//...................................//for all centroid of hull...................//for all.centroid of nearest point of centroid ............
        ...
        float** centroid_original = new float* [1200];
        ...
        for (argg = 1;argg < 3;argg++)
        {
            ...
            pcl::io::loadPCDFile<pcl::PointXYZ>(this->files[argg], *cloud_ptr); //////argv[1]
            pcl::io::loadPCDFile<pcl::PointXYZRGB>(this->files[argg], *befor_clustering_cloud);
            ...
            std::cout << "Loaded pcd file after sampling" << this->files[argg] << " with " << cloud_ptr->points.size() << std::endl;
    ...
    */
            ...
            // ADIING POSE TO MSG
            object.position.x = mean_cloud_in_x / count_metched;
            object.position.y = mean_cloud_in_y / count_metched;
            object.position.z = mean_cloud_in_z / count_metched;
        ...
        //////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            ROS_INFO_STREAM("Deleted 1");
            // deallocate memory using delete[] operator
            for (int i = 0; i < M; i++)
                delete[] data2[i];

            delete[] data2;
            ROS_INFO_STREAM("Deleted 2");
            /////////////////////////////////////////////////////////////
            for (int i = 0; i <1200; ++i)
                //                  ^^^^^ not levelSize.x
            {
