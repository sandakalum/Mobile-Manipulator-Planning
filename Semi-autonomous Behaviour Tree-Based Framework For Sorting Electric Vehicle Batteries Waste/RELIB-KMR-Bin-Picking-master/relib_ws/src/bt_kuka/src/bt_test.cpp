...
public:
	...
	void sendRequest(RequestType& request) override
	{
		...
		temp_pose.header.frame_id = GLOBAL_FRAME;
		request.obj_pose = temp_pose;
...
public:
	DetectObj(ros::NodeHandle &handle, const std::string &node_name, const NodeConfiguration &conf) : RosServiceNode<gazebo_msgs::GetModelState>(handle, node_name, conf)
	{
		// node_handle = handle;
...
public:
	...
	void sendRequest(RequestType &request) override
	{
		...
		request.place_pose.pose.position.x = tf2_bin_position.getX();
		request.place_pose.pose.position.y = tf2_bin_position.getY();
		request.place_pose.pose.position.z = tf2_bin_position.getZ();
...
//                     <Action ID="DetectObj" obj_name="object_a" pose="{object_pose}" service_name="gazebo/get_model_state"/>
//                     <Action ID="Grasp" object_pose="{object_pose}" service_name="grasp"/>
//                     <Action ID="NavigateTo" base_pose="{base_pose}" server_name="move_base" ws="2"/>
//                     <Sequence name="place_obj">
//                         <Action ID="Place" bin_number="{bin}" service_name="place_2"/>
//                     </Sequence>
//                     <Action ID="LookAtTable" pose="eye" service_name="arm_pose" status="{succes}"/>
//                 </Sequence>
//             </Fallback>
